// Toyota Connected Services API Type Definitions
//
// Pure data models for Toyota API integration with zero dependencies
// on Spin SDK. All types are Serde-compatible for JSON serialization.

#[allow(warnings)]
mod bindings;

use bindings::exports::toyota::api_types::models::Guest;
use serde::{Deserialize, Serialize};
use std::{convert::From, option::Option};

// =============================================================================
// OAuth2 Authentication Structures
// =============================================================================
...
impl Guest for Component {
    ...
    fn deserialize_token_response(json: String) -> Result<(String, String, String, u32), String> {
        match serde_json::from_str::<TokenResponse>(&json) {
            ...
            Err(e) => Err(format!("Failed to deserialize TokenResponse: {}", e)),
        }
    }
    ...
    fn serialize_electric_status(
        ...
        let response = ElectricStatusResponse {
            payload: ElectricStatusPayload {
                vehicle_info: ElectricVehicleInfo {
                    charge_info: NewChargeInfo {
                        charge_remaining_amount: battery_level,
                        charging_status,
                        ev_range,
                        ev_range_with_ac,
    ...
    fn deserialize_vehicle_list(json: String) -> Result<String, String> {
        match serde_json::from_str::<VehicleListResponse>(&json) {
            Ok(_) => Ok(json), // Return original JSON if valid
            Err(e) => Err(format!("Failed to deserialize VehicleListResponse: {}", e)),
        }
    }

    fn serialize_location(lat: f64, lon: f64, timestamp: String) -> String {
    ...
    fn deserialize_location(json: String) -> Result<(f64, f64, String), String> {
        match serde_json::from_str::<LocationResponse>(&json) {
            Ok(loc) => Ok((
                loc.payload.vehicle_info.location.lat,
                loc.payload.vehicle_info.location.lon,
                loc.payload.vehicle_info.last_update_timestamp,
            )),
            Err(e) => Err(format!("Failed to deserialize LocationResponse: {}", e)),
        }
    }
    ...
    fn create_token_request(code: String) -> String {
        let req = TokenRequest::new(code);
        serde_json::to_string(&req).unwrap_or_default()
    }

    fn create_refresh_request(refresh_token: String) -> String {
        let req = RefreshTokenRequest::new(refresh_token);
        serde_json::to_string(&req).unwrap_or_default()
    }
